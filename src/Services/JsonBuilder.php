...
class JsonBuilder
{
    public function stockData(Equipe $equipe)
    {
        ...
        $data = [];
        $data['personnage'] = ['Element' => $personnage->getElement()->getNameElement(), 'Type' => $personnage->getTypeWeapon()->getNameType(), 'Category' => $personnage->getCategoryWeapon()->getNameCategory(), 'Attack' => $personnage->getAttack(), 'Magic' => $personnage->getMagic(), 'Life' => $personnage->getLife(),  'Energy' => $personnage->getEnergy()];
        $data['weapon'] = ['ElementBase' => $weapon->getWeaponElement()->getNameElement(), 'ElementSkill' => $weapon->getSkillElement()->getNameElement(), 'Type' => $personnage->getTypeWeapon()->getNameType(), 'Category' => $weapon->getCategoryWeapon()->getNameCategory(), 'baseAttack' => $weapon->getAttackStat(), 'baseMagic' => $weapon->getMagicStat(), 'skillAttack' => $weapon->getAttackSkill(), 'skillMagic' => $weapon->getMagicSkill(), 'EnergyCost' => $weapon->getEnergySkill(), 'turnWait' => $weapon->getWaitSkill(), 'skillName' => $weapon->getNameSkill()];
